...
- 创建表
  ...
  auto_increment 自增长，只对int型主键起作用，必须为主键
...
- 字段限制
  ...
  ~~~
  primary key 不允许空值，唯一（只能有一个主键）
  not  null 非空
  unique 唯一可以为空，而且可以为多个
  default  缺省,默认值
  ~~~
...
基本结构： select  字段名列表   from 表名  字段（可以是常量，*，表达式）
...
- 关系运算
  ...
  关系运算符：> 、 >=、  <、  <=、  =、!=、<>、 between and（大于等于，小于等于,包头包尾）
...
limit  offset,n #从第offset条开始取，取n条
	...
	select * from php_user_history limit 4,2; #limit offset,n
